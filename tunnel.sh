...
<title>Youzin Crabz — VPN Panel</title>
<style>
@import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@700;900&family=Exo+2:wght@300;400;600&family=Share+Tech+Mono&display=swap');
*,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
:root{
  --c1:#00d4ff;--c2:#7ee8fa;--c3:#00ff88;--c4:#f4c01e;--c5:#ff4757;--c6:#a855f7;
  --bg:#03050e;--bg2:#080d1a;--bg3:#0d1528;
}
html{scroll-behavior:smooth}
body{background:var(--bg);color:#cce7ff;font-family:'Exo 2',sans-serif;min-height:100vh;overflow-x:hidden}
...
.wrap{position:relative;z-index:1;max-width:920px;margin:0 auto;padding:0 18px 80px}

/* ── HERO ── */
.hero{text-align:center;padding:70px 20px 44px;position:relative}
.hero-glow{position:absolute;top:0;left:50%;transform:translateX(-50%);width:600px;height:400px;background:radial-gradient(ellipse,rgba(0,212,255,.1) 0%,transparent 70%);pointer-events:none;animation:breathe 5s ease-in-out infinite}
@keyframes breathe{0%,100%{opacity:.6}50%{opacity:1}}

/* LOGO RING */
.logo-wrap{position:relative;display:inline-block;margin-bottom:36px}
.logo-ring{width:110px;height:110px;border-radius:50%;border:2px solid var(--c1);display:flex;align-items:center;justify-content:center;font-size:3rem;position:relative;animation:ringPulse 3s ease-in-out infinite;margin:0 auto}
.logo-ring::before,.logo-ring::after{content:'';position:absolute;border-radius:50%;border:1px solid rgba(0,212,255,.25);animation:ringExpand 3s ease-out infinite}
.logo-ring::before{width:134px;height:134px;animation-delay:0s}
.logo-ring::after{width:158px;height:158px;animation-delay:1.2s}
@keyframes ringExpand{0%{opacity:.7;transform:scale(.88)}100%{opacity:0;transform:scale(1.38)}}
@keyframes ringPulse{0%,100%{box-shadow:0 0 20px var(--c1),inset 0 0 16px rgba(0,212,255,.08)}50%{box-shadow:0 0 55px var(--c1),0 0 110px rgba(0,212,255,.25),inset 0 0 36px rgba(0,212,255,.12)}}

/* BRAND NAME — bold + wave animation */
.brand{font-family:'Orbitron',monospace;font-weight:900;font-size:clamp(2.2rem,7vw,3.6rem);letter-spacing:4px;text-transform:uppercase;display:inline-flex;gap:0;justify-content:center;flex-wrap:wrap}
.brand span{display:inline-block;background:linear-gradient(135deg,#ffffff,var(--c1),var(--c6),var(--c2));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;animation:waveChar 2.4s ease-in-out infinite}
.brand span:nth-child(1){animation-delay:.00s}
.brand span:nth-child(2){animation-delay:.06s}
.brand span:nth-child(3){animation-delay:.12s}
.brand span:nth-child(4){animation-delay:.18s}
.brand span:nth-child(5){animation-delay:.24s}
.brand span:nth-child(6){animation-delay:.30s}
.brand span:nth-child(7){animation-delay:.36s}
.brand span:nth-child(8){animation-delay:.42s}
.brand span:nth-child(9){animation-delay:.48s}
.brand span:nth-child(10){animation-delay:.54s}
.brand span:nth-child(11){animation-delay:.60s}
.brand span:nth-child(12){animation-delay:.66s}
@keyframes waveChar{0%,100%{transform:translateY(0)}40%{transform:translateY(-10px)}}

.brand-sub{font-family:'Share Tech Mono',monospace;font-size:.82rem;letter-spacing:8px;color:var(--c2);text-transform:uppercase;margin-top:10px;opacity:.75}

/* TYPING */
.typing-wrap{min-height:30px;margin:20px 0 12px}
.typing{font-family:'Share Tech Mono',monospace;font-size:.95rem;color:var(--c1);letter-spacing:2px}
.cursor{display:inline-block;width:2px;height:1em;background:var(--c1);animation:blink .65s step-end infinite;vertical-align:middle;margin-left:2px}
@keyframes blink{0%,100%{opacity:1}50%{opacity:0}}

.badge{display:inline-flex;align-items:center;gap:8px;margin-top:8px;background:rgba(0,212,255,.06);border:1px solid rgba(0,212,255,.2);border-radius:30px;padding:7px 22px;font-size:.75rem;color:var(--c2);letter-spacing:2px}
.badge-dot{width:8px;height:8px;border-radius:50%;background:var(--c3);box-shadow:0 0 8px var(--c3);animation:pdot 1.6s ease-in-out infinite}
@keyframes pdot{0%,100%{transform:scale(1)}50%{transform:scale(1.5)}}

/* ── TICKER ── */
.ticker-outer{overflow:hidden;background:rgba(0,212,255,.04);border-top:1px solid rgba(0,212,255,.12);border-bottom:1px solid rgba(0,212,255,.12);padding:10px 0;margin:22px 0 34px}
.ticker-inner{display:flex;animation:tickRun 32s linear infinite;white-space:nowrap}
.ticker-inner:hover{animation-play-state:paused}
.tick-item{font-family:'Share Tech Mono',monospace;font-size:.78rem;color:var(--c2);padding:0 44px;flex-shrink:0}
.tick-item b{color:var(--c1)}
@keyframes tickRun{0%{transform:translateX(0)}100%{transform:translateX(-50%)}}

/* ── SECTION TITLE ── */
.sec-title{font-family:'Orbitron',monospace;font-size:.7rem;color:var(--c1);letter-spacing:4px;text-transform:uppercase;margin:32px 0 14px;display:flex;align-items:center;gap:12px}
.sec-title::after{content:'';flex:1;height:1px;background:linear-gradient(90deg,rgba(0,212,255,.5),transparent)}

/* ── CARDS ── */
.cards{display:grid;grid-template-columns:repeat(auto-fit,minmax(195px,1fr));gap:14px;margin-bottom:14px}
.card{background:var(--bg3);border:1px solid rgba(0,212,255,.15);border-radius:14px;padding:20px 16px;position:relative;overflow:hidden;transition:all .3s}
.card::before{content:'';position:absolute;top:0;left:-100%;width:100%;height:2px;background:linear-gradient(90deg,transparent,var(--c1),transparent);animation:scanLine 4s linear infinite}
@keyframes scanLine{0%{left:-100%}100%{left:100%}}
.card:hover{border-color:var(--c1);transform:translateY(-5px);box-shadow:0 10px 36px rgba(0,212,255,.14)}
.card-icon{font-size:1.9rem;margin-bottom:10px}
.card-label{font-family:'Orbitron',monospace;font-size:.6rem;color:var(--c1);letter-spacing:2px;text-transform:uppercase;margin-bottom:8px}
.card-val{font-family:'Share Tech Mono',monospace;font-size:.84rem;line-height:1.7;color:#ddeeff}
.hi{color:var(--c3)}.lo{color:var(--c2);font-size:.76rem}

/* ── STATUS GRID ── */
.status-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(128px,1fr));gap:10px;margin-bottom:14px}
.svc{background:var(--bg2);border:1px solid rgba(0,212,255,.1);border-radius:10px;padding:11px 14px;display:flex;align-items:center;gap:10px;font-family:'Share Tech Mono',monospace;font-size:.8rem}
.dot{width:9px;height:9px;border-radius:50%;flex-shrink:0}
.dot.on{background:var(--c3);box-shadow:0 0 8px var(--c3);animation:pdot 2s ease-in-out infinite}
.dot.off{background:var(--c5);box-shadow:0 0 6px var(--c5)}

/* ── INFO BOX ── */
.infobox{background:var(--bg2);border:1px solid rgba(0,212,255,.12);border-radius:14px;padding:22px;margin-bottom:14px}
.infobox p{font-size:.84rem;color:#99bbdd;line-height:1.9}
.infobox strong{color:var(--c2)}

/* ── WARNING BOXES ── */
.warn-box{border-radius:14px;padding:20px 22px;margin-bottom:14px;position:relative;overflow:hidden}
.warn-box::before{content:'';position:absolute;inset:0;pointer-events:none}
.warn-box.red{background:rgba(255,71,87,.04);border:1px solid rgba(255,71,87,.28)}
.warn-box.red::before{background:linear-gradient(135deg,rgba(255,71,87,.04),transparent)}
.warn-box.yellow{background:rgba(244,192,30,.04);border:1px solid rgba(244,192,30,.22)}
.warn-box.yellow::before{background:linear-gradient(135deg,rgba(244,192,30,.04),transparent)}
.warn-box.blue{background:rgba(0,212,255,.04);border:1px solid rgba(0,212,255,.22)}
.warn-box.blue::before{background:linear-gradient(135deg,rgba(0,212,255,.04),transparent)}
.warn-title{font-family:'Orbitron',monospace;font-size:.65rem;letter-spacing:3px;text-transform:uppercase;margin-bottom:14px;display:flex;align-items:center;gap:8px}
.warn-box.red .warn-title{color:var(--c5)}
.warn-box.yellow .warn-title{color:var(--c4)}
.warn-box.blue .warn-title{color:var(--c1)}
.warn-box ul{list-style:none}
.warn-box li{font-size:.82rem;color:#bbccdd;padding:5px 0 5px 28px;position:relative;line-height:1.7;border-bottom:1px solid rgba(255,255,255,.04)}
.warn-box li:last-child{border-bottom:none}
.warn-box.red li::before{content:'🚫';position:absolute;left:0;font-size:.76rem;top:5px}
.warn-box.yellow li::before{content:'⚡';position:absolute;left:2px;top:5px;color:var(--c4)}
.warn-box.blue li::before{content:'ℹ️';position:absolute;left:0;font-size:.76rem;top:5px}

/* ── KENDALA / CONTACT BOX ── */
.contact-box{background:linear-gradient(135deg,rgba(0,212,255,.05),rgba(168,85,247,.05));border:1px solid rgba(0,212,255,.25);border-radius:16px;padding:28px 24px;margin-bottom:14px;text-align:center;position:relative;overflow:hidden}
.contact-box::after{content:'';position:absolute;inset:0;background:radial-gradient(ellipse at top center,rgba(0,212,255,.07),transparent 60%);pointer-events:none}
.contact-title{font-family:'Orbitron',monospace;font-size:.8rem;color:var(--c1);letter-spacing:3px;text-transform:uppercase;margin-bottom:8px}
.contact-desc{font-size:.84rem;color:#8899bb;line-height:1.8;margin-bottom:20px}
.tg-btn{display:inline-flex;align-items:center;gap:10px;background:linear-gradient(135deg,#0088cc,#0055aa);color:#fff;padding:13px 38px;border-radius:40px;font-family:'Orbitron',monospace;font-size:.78rem;letter-spacing:2px;text-decoration:none;box-shadow:0 4px 28px rgba(0,136,204,.4);transition:all .3s;position:relative;overflow:hidden}
.tg-btn::before{content:'';position:absolute;top:0;left:-100%;width:100%;height:100%;background:linear-gradient(90deg,transparent,rgba(255,255,255,.12),transparent);transition:.5s}
.tg-btn:hover{transform:translateY(-3px);box-shadow:0 10px 44px rgba(0,136,204,.6)}
.tg-btn:hover::before{left:100%}
.tg-handle{font-family:'Share Tech Mono',monospace;font-size:.9rem;color:var(--c1);margin-top:14px;letter-spacing:3px}

/* ── DIVIDER ── */
.div{height:1px;background:linear-gradient(90deg,transparent,var(--c1),transparent);margin:30px 0;animation:divPulse 4s ease-in-out infinite}
@keyframes divPulse{0%,100%{opacity:.25}50%{opacity:.7}}

/* ── FOOTER ── */
footer{text-align:center;padding:28px 0 10px;font-family:'Share Tech Mono',monospace;font-size:.68rem;color:#1e3050;letter-spacing:1px}
footer strong{color:#2a4565}
#clkLine{color:#1a2a40;margin-top:5px}

@media(max-width:520px){
  .brand{font-size:1.9rem;letter-spacing:2px}
...
<div class="hero">
  ...
  <div class="brand">
    <span>Y</span><span>o</span><span>u</span><span>z</span><span>i</span><span>n</span>&nbsp;<span>C</span><span>r</span><span>a</span><span>b</span><span>z</span>
  </div>
  <div class="brand-sub">Tunnel &nbsp;·&nbsp; Secure VPN Panel</div>
  <div class="typing-wrap">
    <span class="typing" id="typTxt"></span><span class="cursor"></span>
  </div>
  <div class="badge">
    <span class="badge-dot"></span>
    <span>✦ &nbsp;by The Professor &nbsp;✦</span>
  </div>
</div>
...
<div class="ticker-outer">
  <div class="ticker-inner">
    <span class="tick-item">⚡ <b>Youzin Crabz</b> — Panel VPN Premium</span>
    <span class="tick-item">🔒 Xray-core: <b>VMess · VLess · Trojan</b></span>
    <span class="tick-item">🌐 TLS <b>443</b> &nbsp;|&nbsp; No-TLS <b>80</b></span>
    <span class="tick-item">🛡 SSH · Dropbear · HAProxy · Nginx</span>
    <span class="tick-item">📡 Uptime <b>24/7</b> — Server Stabil</span>
    <span class="tick-item">✈ Telegram: <b>@YouzinCrabz</b></span>
    <span class="tick-item">🦀 <b>The Professor</b> — All rights reserved</span>
    <span class="tick-item">⚡ <b>Youzin Crabz</b> — Panel VPN Premium</span>
    <span class="tick-item">🔒 Xray-core: <b>VMess · VLess · Trojan</b></span>
    <span class="tick-item">🌐 TLS <b>443</b> &nbsp;|&nbsp; No-TLS <b>80</b></span>
    <span class="tick-item">🛡 SSH · Dropbear · HAProxy · Nginx</span>
...
<!-- PROTOCOL CARDS -->
<div class="sec-title">⬡ Protocol &amp; Port</div>
<div class="cards">
  <div class="card">
    <div class="card-icon">🌐</div>
    <div class="card-label">Protocol</div>
    <div class="card-val"><span class="hi">VMess</span> · <span class="hi">VLess</span> · <span class="hi">Trojan</span><br><span class="lo">SSH · Dropbear</span></div>
  </div>
  <div class="card">
    <div class="card-icon">🔗</div>
...
<!-- SERVICES -->
<div class="sec-title">⬡ Network Services</div>
<div class="status-grid">
  <div class="svc"><div class="dot on"></div>XRAY</div>
  <div class="svc"><div class="dot on"></div>NGINX</div>
  <div class="svc"><div class="dot on"></div>HAPROXY</div>
  <div class="svc"><div class="dot on"></div>DROPBEAR</div>
  <div class="svc"><div class="dot on"></div>SSH</div>
  <div class="svc"><div class="dot on"></div>UDP CUSTOM</div>
</div>

<div class="div"></div>

<!-- ABOUT -->
<div class="sec-title">⬡ Tentang Panel</div>
<div class="infobox">
  <p>
    <strong>Youzin Crabz</strong> adalah panel VPN premium berbasis <strong>Xray-core</strong>
    ...
    Script 100% dibangun oleh <strong>The Professor</strong>.
  </p>
</div>

<!-- PERINGATAN MERAH -->
<div class="sec-title">⬡ Peringatan &amp; Ketentuan</div>
<div class="warn-box red">
  <div class="warn-title">🚫 Dilarang Keras</div>
  <ul>
    <li>Menjual, mendistribusikan, atau menyebarkan script ini tanpa izin resmi dari <strong>The Professor</strong>.</li>
    <li>Menghapus, mengganti, atau memodifikasi signature dan kredit dalam script.</li>
    <li>Mengklaim script ini sebagai karya sendiri (<em>re-branding</em> tanpa izin).</li>
...
<!-- PERINGATAN KUNING -->
<div class="warn-box yellow">
  <div class="warn-title">⚡ Penting — Baca Sebelum Menggunakan</div>
  <ul>
    <li>Hak cipta sepenuhnya milik <strong>The Professor / Youzin Crabz</strong>. All rights reserved.</li>
...
<div class="warn-box blue">
  <div class="warn-title">ℹ️ Informasi Penggunaan</div>
  <ul>
    ...
    <li>Jangan share akun ke orang lain — setiap akun hanya untuk 1 pengguna.</li>
  </ul>
</div>

<div class="div"></div>

<!-- KENDALA HUBUNGI TELEGRAM -->
<div class="contact-box">
  <div class="contact-title">💬 Ada Kendala?</div>
  <div class="contact-desc">
    Jika mengalami masalah instalasi, akun tidak bisa konek,<br>
    error konfigurasi, atau pertanyaan teknis lainnya —<br>
...
<footer>
  <p>© 2024–2025 <strong>Youzin Crabz</strong> — The Professor &nbsp;·&nbsp; All rights reserved</p>
  <p id="clkLine">[ __ ] Powered by Xray-core &amp; Nginx</p>
</footer>
...
(function(){
  ...
  const mkP=()=>({x:Math.random()*W,y:Math.random()*H,vx:(Math.random()-.5)*.35,vy:(Math.random()-.5)*.35,r:Math.random()*1.4+.4,a:Math.random()*.6+.2});
  for(let i=0;i<90;i++)pts.push(mkP());
  (function draw(){
    ...
    for(let i=0;i<pts.length;i++)for(let j=i+1;j<pts.length;j++){
      ...
      if(d<110){ctx.beginPath();ctx.moveTo(pts[i].x,pts[i].y);ctx.lineTo(pts[j].x,pts[j].y);ctx.strokeStyle=`rgba(0,212,255,${.12*(1-d/110)})`;ctx.lineWidth=.4;ctx.stroke();}
    }
...
/* TYPING */
(function(){
  const msgs=['Secure · Fast · Reliable','VMess · VLess · Trojan','SSH · Dropbear · UDP','by The Professor','@YouzinCrabz','Uptime 24/7'];
  let mi=0,ci=0,del=false;
  const el=document.getElementById('typTxt');
  (function tick(){
    const m=msgs[mi];
    if(!del){el.textContent=m.slice(0,++ci);if(ci===m.length){del=true;setTimeout(tick,1900);return;}}
    else{el.textContent=m.slice(0,--ci);if(ci===0){del=false;mi=(mi+1)%msgs.length;setTimeout(tick,400);return;}}
...
_ordervpn_setup_nginx() {
    ...
    local DIR="/var/www/html/ordervpn"
    local PHP_SOCK=""
    for sock in /var/run/php/php*.fpm.sock; do [[ -S "$sock" ]] && { PHP_SOCK="$sock"; break; }; done
...
menu_ordervpn() {
    ...
    while true; do
        ...
        printf "  ${WHITE}[2]${NC} Test vpn-api bridge\n"
        ...
        case $ovpn_choice in
            ...
            2)
                ...
                if [[ -x /usr/local/bin/vpn-api ]]; then
                    ...
                    echo ""; printf "  ${DIM}Contoh: vpn-api create vmess user30 30 100 2${NC}\n"
                else
            ...
            4)
                ...
                [[ -f "$LOG" ]] && tail -60 "$LOG" || echo -e "  ${DIM}Log belum ada${NC}"
                echo ""; read -p "  Tekan ENTER..." ;;
            5)
                echo ""; read -p "  Masukkan subdomain (contoh: order.domain.com): " subdomain
                [[ -n "$subdomain" ]] && _ordervpn_setup_nginx 8888 "$subdomain" && echo -e "  ${GREEN}✔ Subdomain $subdomain aktif${NC}"
                sleep 2 ;;
            6)
                ...
                if [[ "${yn,,}" == "y" ]]; then
                    ...
                    nginx -t 2>/dev/null && systemctl reload nginx 2>/dev/null
                    echo -e "  ${GREEN}✔ OrderVPN diuninstall${NC}"; sleep 2
                fi ;;
            0) return ;;
        esac
    done
...
SQLEOF
    ...
    [[ -d /var/www/html/ordervpn ]] && mv /var/www/html/ordervpn /var/www/html/ordervpn.bak 2>/dev/null
