...
<title>${PAGE_TITLE} — VPN Premium Panel</title>
...
:root{
  --neon:#00d4ff;--neon2:#7c3aed;--neon3:#10b981;--neon4:#f59e0b;
  --bg:#03070f;--bg2:#060e1c;--bg3:#0b1628;--bg4:#0f1e36;
  --border:rgba(0,212,255,.1);--border2:rgba(124,58,237,.15);
  --text:#e2f0ff;--muted:#3d5a7a;--soft:#8ab0cc;
}
...
.glow-orb-1{width:600px;height:600px;background:radial-gradient(circle,rgba(0,212,255,.07),transparent 70%);top:-200px;left:-200px;animation:orb1 20s ease-in-out infinite alternate}
.glow-orb-2{width:500px;height:500px;background:radial-gradient(circle,rgba(124,58,237,.06),transparent 70%);bottom:-150px;right:-150px;animation:orb2 16s ease-in-out infinite alternate}
@keyframes orb1{to{transform:translate(100px,80px)}}
@keyframes orb2{to{transform:translate(-80px,-60px)}}

.wrap{max-width:960px;margin:0 auto;padding:0 20px 80px;position:relative;z-index:1}

/* === HEADER NAV === */
.nav{display:flex;align-items:center;justify-content:space-between;padding:28px 0 20px}
.nav-brand{display:flex;align-items:center;gap:10px}
.nav-dot{width:10px;height:10px;border-radius:50%;background:var(--neon);box-shadow:0 0 16px var(--neon);animation:pulse 2s ease-in-out infinite}
@keyframes pulse{0%,100%{transform:scale(1);opacity:1}50%{transform:scale(1.6);opacity:.6}}
.nav-name{font-size:1rem;font-weight:700;letter-spacing:.05em;color:var(--text)}
.nav-badge{font-family:'DM Mono',monospace;font-size:.65rem;background:rgba(0,212,255,.08);border:1px solid rgba(0,212,255,.2);border-radius:20px;padding:3px 10px;color:var(--neon);letter-spacing:.1em}

/* === HERO === */
.hero{padding:60px 0 52px;text-align:center;position:relative}
.hero-kicker{font-family:'DM Mono',monospace;font-size:.7rem;letter-spacing:.25em;color:var(--neon);text-transform:uppercase;margin-bottom:20px;display:inline-flex;align-items:center;gap:8px}
.hero-kicker::before,.hero-kicker::after{content:'';width:30px;height:1px;background:linear-gradient(90deg,transparent,var(--neon))}
.hero-kicker::after{background:linear-gradient(90deg,var(--neon),transparent)}
.hero-title{font-size:clamp(2.4rem,7vw,4.5rem);font-weight:800;line-height:1.05;letter-spacing:-.02em;margin-bottom:20px}
.hero-title .hl{background:linear-gradient(135deg,var(--neon) 0%,var(--neon2) 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
.hero-sub{font-size:clamp(.85rem,2.5vw,1rem);color:var(--soft);max-width:520px;margin:0 auto 32px;line-height:1.8;font-weight:400}
.hero-status{display:inline-flex;align-items:center;gap:8px;background:rgba(16,185,129,.06);border:1px solid rgba(16,185,129,.2);border-radius:30px;padding:7px 20px;font-family:'DM Mono',monospace;font-size:.72rem;color:#6ee7b7;letter-spacing:.05em}

/* === MARQUEE TICKER === */
.ticker{overflow:hidden;border-top:1px solid var(--border);border-bottom:1px solid var(--border);background:rgba(0,212,255,.02);padding:11px 0;margin:36px 0}
.ticker-track{display:flex;width:max-content;animation:marquee 30s linear infinite}
.ticker-track:hover{animation-play-state:paused}
.tick-item{font-family:'DM Mono',monospace;font-size:.72rem;color:var(--muted);padding:0 44px;white-space:nowrap}
.tick-item b,.tick-sep{color:var(--neon)}
.tick-sep{padding:0 4px}
@keyframes marquee{0%{transform:translateX(0)}100%{transform:translateX(-50%)}}

/* === SECTION HEADING === */
.sec{display:flex;align-items:center;gap:12px;margin:40px 0 16px}
.sec-label{font-family:'DM Mono',monospace;font-size:.65rem;color:var(--neon);letter-spacing:.22em;text-transform:uppercase;white-space:nowrap}
.sec-line{flex:1;height:1px;background:linear-gradient(90deg,var(--border),transparent)}

/* === GLASS CARDS === */
.cards{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:10px;margin-bottom:10px}
.card{background:rgba(6,14,28,.7);border:1px solid var(--border);border-radius:16px;padding:20px 18px;position:relative;overflow:hidden;transition:all .3s;backdrop-filter:blur(8px)}
.card::before{content:'';position:absolute;inset:0;border-radius:16px;background:linear-gradient(135deg,rgba(0,212,255,.04),transparent 60%);opacity:0;transition:opacity .3s}
.card:hover{border-color:rgba(0,212,255,.3);transform:translateY(-5px);box-shadow:0 12px 40px rgba(0,212,255,.07)}
.card:hover::before{opacity:1}
.card-icon{width:36px;height:36px;border-radius:10px;background:rgba(0,212,255,.08);border:1px solid rgba(0,212,255,.15);display:flex;align-items:center;justify-content:center;margin-bottom:14px;font-size:1rem}
.card-label{font-family:'DM Mono',monospace;font-size:.58rem;color:var(--muted);letter-spacing:.2em;text-transform:uppercase;margin-bottom:6px}
.card-val{font-family:'DM Mono',monospace;font-size:.83rem;color:var(--soft);line-height:1.9}
.card-val .hi{color:var(--neon);font-weight:500}
.card-val .vi{color:#a78bfa}
.card-val .gi{color:var(--neon3)}

/* === SERVICE STATUS GRID === */
.svc-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:8px;margin-bottom:10px}
.svc-item{background:rgba(6,14,28,.8);border:1px solid var(--border);border-radius:12px;padding:11px 14px;display:flex;align-items:center;gap:9px;transition:border-color .3s}
.svc-item:hover{border-color:var(--border2)}
.dot{width:8px;height:8px;border-radius:50%;flex-shrink:0;transition:box-shadow .3s}
.dot.on{background:var(--neon3);box-shadow:0 0 8px var(--neon3);animation:pulse 2.2s ease-in-out infinite}
.dot.off{background:#ef4444;box-shadow:0 0 5px rgba(239,68,68,.5)}
.dot.chk{background:var(--neon4);box-shadow:0 0 6px var(--neon4);animation:pulse 1.5s ease-in-out infinite}
.svc-name{font-family:'DM Mono',monospace;font-size:.72rem;color:var(--soft);letter-spacing:.04em}

/* === INFO PANEL === */
.info-panel{background:rgba(6,14,28,.7);border:1px solid var(--border);border-radius:16px;padding:22px;margin-bottom:10px;backdrop-filter:blur(8px)}
.info-panel p{font-size:.85rem;color:var(--soft);line-height:1.9}
.info-panel strong{color:var(--neon)}

/* === RULES CARDS === */
.rules{display:grid;grid-template-columns:1fr;gap:8px;margin-bottom:10px}
.rule-card{border-radius:14px;padding:18px 20px}
.rule-card.danger{background:rgba(239,68,68,.03);border:1px solid rgba(239,68,68,.15)}
.rule-card.warn{background:rgba(245,158,11,.03);border:1px solid rgba(245,158,11,.12)}
.rule-card.info{background:rgba(0,212,255,.03);border:1px solid rgba(0,212,255,.12)}
.rule-head{font-family:'DM Mono',monospace;font-size:.6rem;letter-spacing:.25em;text-transform:uppercase;font-weight:600;margin-bottom:12px}
.rule-card.danger .rule-head{color:#f87171}
.rule-card.warn .rule-head{color:var(--neon4)}
.rule-card.info .rule-head{color:var(--neon)}
.rule-list{list-style:none}
.rule-list li{font-size:.82rem;color:#7a9ab5;padding:5px 0 5px 20px;position:relative;line-height:1.7;border-bottom:1px solid rgba(255,255,255,.03)}
.rule-list li:last-child{border-bottom:none}
.rule-card.danger li::before{content:'×';position:absolute;left:0;color:#f87171;font-weight:700}
.rule-card.warn li::before{content:'!';position:absolute;left:3px;color:var(--neon4);font-weight:700}
.rule-card.info li::before{content:'›';position:absolute;left:2px;color:var(--neon)}

/* === CONTACT === */
.contact{background:linear-gradient(135deg,rgba(0,212,255,.04),rgba(124,58,237,.04));border:1px solid rgba(0,212,255,.15);border-radius:18px;padding:28px 24px;text-align:center;position:relative;overflow:hidden}
.contact::before{content:'';position:absolute;top:-50%;left:-50%;width:200%;height:200%;background:radial-gradient(circle at 50% 50%,rgba(0,212,255,.04),transparent 60%);pointer-events:none}
.contact-tag{font-family:'DM Mono',monospace;font-size:.65rem;color:var(--neon);letter-spacing:.2em;text-transform:uppercase;margin-bottom:8px}
.contact-title{font-size:1.3rem;font-weight:700;color:var(--text);margin-bottom:8px}
.contact-desc{font-size:.84rem;color:var(--soft);line-height:1.9;margin-bottom:22px}
.tg-btn{display:inline-flex;align-items:center;gap:10px;background:linear-gradient(135deg,#0088cc,#0055aa);color:#fff;padding:12px 32px;border-radius:40px;font-family:'DM Mono',monospace;font-size:.75rem;letter-spacing:.08em;text-decoration:none;box-shadow:0 4px 24px rgba(0,136,204,.3);transition:all .3s;position:relative;z-index:1}
.tg-btn:hover{transform:translateY(-3px);box-shadow:0 10px 36px rgba(0,136,204,.45)}
.tg-handle{font-family:'DM Mono',monospace;font-size:.78rem;color:var(--neon);margin-top:14px;letter-spacing:.1em;opacity:.7}

/* === DIVIDER === */
.divider{height:1px;background:linear-gradient(90deg,transparent,var(--border),transparent);margin:32px 0;opacity:.6}

/* === FOOTER === */
footer{text-align:center;padding:28px 0 12px;font-family:'DM Mono',monospace;font-size:.62rem;color:#0e2040;letter-spacing:.05em}
footer strong{color:#142840}
#clk{color:#0a1a30;margin-top:5px}
...
<nav class="nav">
  <div class="nav-brand">
    ...
    <span class="nav-name">${PAGE_TITLE}</span>
  </div>
  <span class="nav-badge">VPN PANEL v3</span>
</nav>

<!-- HERO -->
<section class="hero">
  <div class="hero-kicker">Youzin Crabz Tunnel</div>
  <h1 class="hero-title">
    <span class="hl">${PAGE_TITLE}</span><br>Premium VPN
  </h1>
  <p class="hero-sub">Panel VPN berbasis Xray-core dengan VMess, VLess, Trojan &amp; SSH. Transport WebSocket &amp; gRPC. TLS 1.3 &mdash; uptime 24/7.</p>
  <div class="hero-status">
    <span class="dot on"></span>
    Server Online &nbsp;&mdash;&nbsp; Uptime 24/7
  </div>
</section>
...
<div class="ticker">
  <div class="ticker-track">
    <span class="tick-item"><b>VMess</b> <span class="tick-sep">·</span> <b>VLess</b> <span class="tick-sep">·</span> <b>Trojan</b> <span class="tick-sep">·</span> SSH <span class="tick-sep">·</span> Dropbear</span>
    <span class="tick-item">TLS <b>443</b> <span class="tick-sep">|</span> NonTLS <b>80</b> <span class="tick-sep">|</span> UDP <b>7100+</b></span>
    <span class="tick-item">Transport: <b>WebSocket</b> &amp; <b>gRPC</b></span>
    <span class="tick-item">HAProxy <span class="tick-sep">·</span> Nginx <span class="tick-sep">·</span> Xray-core</span>
    <span class="tick-item">Let&apos;s Encrypt <b>TLS 1.3</b></span>
...
<!-- PROTOCOL & PORTS -->
<div class="sec"><span class="sec-label">Protocol &amp; Port</span><span class="sec-line"></span></div>
<div class="cards">
  <div class="card">
    <div class="card-icon">⚡</div>
    <div class="card-label">Protocol</div>
    <div class="card-val"><span class="hi">VMess</span> · <span class="vi">VLess</span><br><span class="gi">Trojan</span> · SSH</div>
  </div>
  <div class="card">
    <div class="card-icon">🔗</div>
    <div class="card-label">Transport</div>
    <div class="card-val"><span class="hi">WebSocket</span> (WS)<br><span class="vi">gRPC</span> Stream</div>
  </div>
  <div class="card">
    <div class="card-icon">🔒</div>
    <div class="card-label">TLS / SSL</div>
    <div class="card-val">TLS <span class="hi">1.2</span> / <span class="hi">1.3</span><br><span class="gi">Let&apos;s Encrypt</span></div>
  </div>
  <div class="card">
    <div class="card-icon">🛰</div>
    <div class="card-label">Port Config</div>
    <div class="card-val">TLS: <span class="hi">443</span> · HTTP: <span class="hi">80</span><br>SSH: <span class="hi">22/222</span> UDP: <span class="hi">7100+</span></div>
  </div>
</div>

<!-- SERVICE STATUS -->
<div class="sec"><span class="sec-label">Network Services</span><span class="sec-line"></span></div>
<div class="svc-grid" id="svcGrid">
  <div class="svc-item"><div class="dot chk" id="dot-xray"></div><span class="svc-name" id="lbl-xray">XRAY</span></div>
  <div class="svc-item"><div class="dot chk" id="dot-nginx"></div><span class="svc-name" id="lbl-nginx">NGINX</span></div>
  <div class="svc-item"><div class="dot chk" id="dot-haproxy"></div><span class="svc-name" id="lbl-haproxy">HAPROXY</span></div>
  <div class="svc-item"><div class="dot chk" id="dot-dropbear"></div><span class="svc-name" id="lbl-dropbear">DROPBEAR</span></div>
  <div class="svc-item"><div class="dot chk" id="dot-ssh"></div><span class="svc-name" id="lbl-ssh">SSH</span></div>
  <div class="svc-item"><div class="dot chk" id="dot-udp"></div><span class="svc-name" id="lbl-udp">UDP CUSTOM</span></div>
</div>

<div class="divider"></div>

<!-- ABOUT -->
<div class="sec"><span class="sec-label">Tentang Panel</span><span class="sec-line"></span></div>
<div class="info-panel">
  <p>
    <strong>${PAGE_TITLE}</strong> adalah panel VPN premium berbasis <strong>Xray-core</strong>
    dengan dukungan <strong>VMess, VLess, dan Trojan</strong> via transport <strong>WebSocket &amp; gRPC</strong>.
...
<!-- RULES -->
<div class="sec"><span class="sec-label">Ketentuan &amp; Peringatan</span><span class="sec-line"></span></div>
<div class="rules">
  <div class="rule-card danger">
    <div class="rule-head">Dilarang Keras</div>
    <ul class="rule-list">
      ...
      <li>Menggunakan panel ini untuk aktivitas ilegal atau merugikan pihak lain.</li>
      <li>Membobol, meretas, atau menyalahgunakan infrastruktur server orang lain.</li>
    </ul>
  </div>
  <div class="rule-card warn">
    <div class="rule-head">Penting &mdash; Baca Sebelum Menggunakan</div>
    <ul class="rule-list">
      <li>Panel ini hanya untuk penggunaan pribadi yang sah dan tidak melanggar hukum setempat.</li>
      <li>Pengguna bertanggung jawab penuh atas segala aktivitas melalui panel ini.</li>
      <li>Admin berhak menonaktifkan akun yang melanggar ketentuan tanpa pemberitahuan.</li>
    </ul>
  </div>
  <div class="rule-card info">
    <div class="rule-head">Informasi Penggunaan</div>
    <ul class="rule-list">
      <li>Akun VPN bersifat sementara sesuai masa aktif yang ditentukan saat pembuatan.</li>
      <li>Pastikan aplikasi client (v2rayNG, Clash, HTTP Custom) terinstall dengan benar.</li>
...
<!-- CONTACT -->
<div class="contact">
  <div class="contact-tag">Support &amp; Bantuan</div>
  <div class="contact-title">Ada Kendala?</div>
  <div class="contact-desc">
    Jika mengalami masalah instalasi, akun tidak bisa konek,<br>
    error konfigurasi, atau pertanyaan teknis &mdash;<br>
    hubungi admin kami langsung di Telegram.
  </div>
...
<footer>
  <p>&copy; 2024&ndash;2025 <strong>${PAGE_TITLE}</strong> &nbsp;&middot;&nbsp; Powered by Xray-core &amp; Nginx</p>
  <p id="clk"></p>
</footer>

<script>
/* === Service Status via /status.json === */
(function(){
  var svcs={
  ...
  function setDot(dotId,state){
    ...
    if(el) el.className='dot '+(state==='active'?'on':'off');
  }
  fetch('/status.json',{cache:'no-cache'})
    ...
    .catch(function(){
      /* Jika status.json tidak ada, tampilkan semua ON */
      Object.values(svcs).forEach(function(v){ setDot(v[0],'active'); });
    });
})();
...
function updateClock(){
  ...
  if(el) el.textContent='[ '+new Date().toLocaleTimeString('id-ID',{hour12:false})+' ] WIB';
}
updateClock();
